...
''' % (basename(__file__), self.trans_port, self.virtual_net, self.trans_port, self.local_dnsport)
        ...
        self.log_file = "../privnet.log"  # Log file name
        self.log = open(self.log_file, "a")  # Create and open the log file

    def __del__(self):
        if self.log:
            self.log.close()  # Close the log file when the program ends
    ...
    def flush_iptables_rules(self):
        ...
        self.write_log("[+] Flushed iptables rules")  # Log
    ...
    def load_iptables_rules(self):
        ...
        self.write_log("[+] Loaded iptables rules")  # Log
...
def main():
    parser = ArgumentParser(
        description=
        'PrivNet is an anonymization tool for loading and unloading iptables rules')
    ...
    try:
        privnet = TorIptables()
        if isfile(privnet.tor_config_file):
            if not 'VirtualAddrNetwork' in open(privnet.tor_config_file).read():
                with open(privnet.tor_config_file, 'a+') as torrconf:
                    torrconf.write(privnet.torrc)

        if args.load:
            privnet.load_iptables_rules()
        elif args.flush:
            privnet.flush_iptables_rules()
            print(" {0}".format("[\033[93m!\033[0m] Anonymizer status \033[0m[OFF]"))
        elif args.refresh:
            privnet.change_ip()
        elif args.ip:
            privnet.get_ip()
